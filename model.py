...
class T5ForMultimodalGenerationMCCoT(T5ForConditionalGeneration):
    ...
    def __init__(self, config: T5Config, patch_size, padding_idx, save_dir,vot_num, alpha, beta):
        ...
        self.beta = beta
        self.ib_model = InformationBottleneck(embedding_size=config.d_model, beta=beta)
    ...
    def load_image_encoder_pretrained(self):
        # self.image_encoder = timm.create_model("vit_large_patch32_384", pretrained=True, num_classes=0)
        config = self.config
        beta = self.beta
        self.image_dense = nn.Linear(self.patch_dim, config.d_model)
        self.mha_layer = torch.nn.MultiheadAttention(embed_dim=config.hidden_size, kdim=config.hidden_size, vdim=config.hidden_size, num_heads=1, batch_first=True)
        ...
        print("Load Image Weight From Scratch")
    ...
    def forward(
        ...
        #TODO: Match Dimmension
        image_embedding = self.image_dense(image_ids)
        
        image_embedding, iba_loss, loss_M = self.ib_model.tokenchoose(image_embedding, hidden_states)
        
        
        image_att, _ = self.mha_layer(hidden_states, image_embedding, image_embedding)


        # ib_loss = self.ib_model.calc_loss(hidden_states, image_embedding)
        ...
        # loss_kl : 0.1
        # loss_M : 0.01
        loss = loss + 0.01 * loss_M + 0.1 * iba_loss
