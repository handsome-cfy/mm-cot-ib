...
def parse_args():
    ...
    parser.add_argument('--vot_num', type=int, default=0)
...
def T5Trainer(
    ...
    if args.img_type is not None:
        ...
        if args.evaluate_dir is None:
            model.load_image_encoder_pretrained()
            console.log(f"[Model]: Model loaded successfully.\n")
    ...
    if args.evaluate_dir is None:
        ...
        wandb.init(
            project="mccot",
            name=f"{args.output_dir}/{args.user_msg}_{model_name}_{args.img_type}_{args.prompt_format}_lr{args.lr}_bs{args.bs * gpu_count}_op{args.output_len}_ep{args.epoch}",
            config=vars(args),
            save_code=True,
            workspace="chenye",
        )
        from utils_log import WandbCallback
